...
def Generate_transition_matrix_med_ent(D = ["a","b","c","d","e"],
    ...
    if n_tranitions < 2:
        print("n_transitions is less than 2")
        sys.exit()
...
#P = Generate_transition_matrix_med_ent(D = ["a","b","c","d","e"],
#                                       e_steps=20,
#                                       n_tranitions=2,
#                                       limit_trials=10)
...
def Generate_transition_matrix_med_ent(D = ["a","b","c","d","e"],
    ...
    if e_steps < len(D):
        print("e_steps should not be less than the size of D")
        sys.exit()
    ...
    #initialization value to start from
    n_max_steps = e_steps + 9999
    n_min_steps = 0
    
    #for test in list(range(0,100)):
    trials = 0
    
    #while desired steps, are less than actual steps, 
    #BUT greater than expected steps-1, keep trying:
    ...
    # WHILE THIS IS NOT TRUE => KEEP SEARCHING
    while (n_min_steps < e_steps-1) or (n_max_steps > e_steps+1):
        
    #while (n_min_steps < 1.0) or (n_max_steps > e_steps) or (n_max_steps < 1.0):
        trials = trials +1
        
        if np.mod(trials, 5) == 0:
            print("Trial no:",trials)
        P = []
        ...
        steps = Get_steps(P,D_orig)
      
        n_max_steps = np.max(steps)
        n_min_steps = np.min(steps)
        
        print("expected steps to END:",steps, "- min steps:",n_min_steps,"- max steps:",n_max_steps, "- target:",e_steps)
        
    if n_min_steps < 0:
        print("ERROR: Negative number of expected steps to absorption.")
        sys.exit()
    
    if trials == limit_trials:
        print("ERROR: Failed to generate P of desired expected stepsize to absorption.")
        sys.exit()
    #generate a dataframe with labels for the "to" states
    P_df = pd.DataFrame(np.reshape(P, newshape=(len(D_orig),len(D_orig))))
    P_df.columns = D_orig
